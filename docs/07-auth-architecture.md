...
3. On success: if this is a device the account hasn't seen before, and the account already has another active device, this does **not** complete immediately any more — see [New-device login approval](#new-device-login-approval) below. Otherwise (an already-known device, or the account's very first device) it proceeds as before: a `devices` row is created from `deviceKeyBundle` (exactly as at invite redemption) and a `security_events` row (`new_device_linked`) is recorded.
...
"Send a notification to the other device to approve it first, only then can the new device log in" — a direct request, shipped as a real gate rather than the passive "surfaced as visible, not silent" notification the original design (above) settled for. A `pending_device_logins` row (`packages/database/prisma/schema.prisma`'s own doc comment carries the full lifecycle) is created instead of registering the device outright, whenever the account already has another active device to approve against — the account's very first device (always reached via invite redemption, never login) has nothing to approve against, so it's unaffected.

Every one of the account's other active devices is notified — live over the realtime socket for one that's already connected, push (`apps/worker`'s dispatcher, same delivery path as a new-message notification) for one that's backgrounded or closed — and shows the request on its own Devices screen with Approve/Deny. Approving or denying never itself completes the new device's login: only the **waiting device's own poll** (`GET /api/auth/login/pending/:id`, repeated every couple of seconds) does that, the moment it observes `approved` — this is deliberate, not incidental: session cookies can only ever be set on the response to the request that will actually use them, never smuggled in from the approving device's own response. A compare-and-swap (`approved` → `completed`) on that same poll is what makes it safe to call concurrently (a slow retry, more than one browser tab) without ever registering the device twice.

A denied or expired (past a several-minute window) request leaves the waiting device permanently unable to complete that attempt — it has to submit its credentials again, generating a fresh request.

## Multi-device message history sync

Asked for directly — "log in anywhere and see your full message history, like WhatsApp" — and genuinely required a structural addition, not a client-side fix: this app's per-device Double Ratchet/Megolm sessions ([05-crypto-architecture](05-crypto-architecture.md)) mean a device only ever has a live decrypt path for messages sent while it was a legitimate target. A brand-new device (or a secondary one added after some history already existed) has no such path for anything older, by the *design* of forward secrecy, not a bug — this is what a fix has to work around, not eliminate.

**The design**: every account has exactly one symmetric History Key (HK), generated client-side the first time any of that account's own devices needs one. HK itself is wrapped (AEAD-encrypted) under a key derived (Argon2id) from the account's own login **password** — the server only ever stores/serves the wrapped bytes (`user_history_keys`), never the raw key or the password used to derive its wrapping key. Any device that knows the password can re-derive the same wrapping key and unwrap HK immediately on login, with **no other device needing to be online** — the deliberate point of difference from a QR-linked-companion history transfer (the WhatsApp/Signal Desktop model), which does the same job but only completes once the primary device happens to be connected.

Every message additionally gets one `message_history_entries` row per participating account: that message's own plaintext, re-encrypted under **that account's** HK, written client-side the moment any one of that account's own devices sees the plaintext by any means (the original sender at send time, or a recipient device the instant it decrypts the message live). Once written, this durably out-survives forward secrecy for that account specifically — any of that account's devices, including ones added far later with no live session of their own, can unwrap HK and recover it. A message neither the sender's nor any recipient's device has decrypted live yet simply has no entry yet, same "no catch-up path until someone's actually online to see it" limitation the live delivery path already has — old history recovers organically as it's scrolled through/received on any already-active device, not via a one-time migration.

**The disclosed trade-off**: HK's confidentiality is bounded by the account password alone — a real, deliberate departure from the per-device-pairwise-session model's much stronger guarantee, made knowingly rather than accidentally. Two things make this a smaller step than it first looks: this deployment's login is already password-based (the server necessarily sees the plaintext password at every login, same as any password-auth system, not a new exposure this feature introduces), and — once multi-device fan-out shipped — a password alone was already enough to add a new device that receives every *future* message via self-fan-out; extending that same bound to *historical* messages closes what was already the practical edge of the account's security, rather than opening a new one. See [08-threat-model](08-threat-model.md)'s updated note.
