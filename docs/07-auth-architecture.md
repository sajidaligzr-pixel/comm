...
3. On success: if this is a device the account hasn't seen before, a `devices` row is created from `deviceKeyBundle` (exactly as at invite redemption) and a `security_events` row (`new_device_linked`) is recorded — login always completes in this same request, immediately, whether or not the account already has other active devices.
...
## New-device login approval — built, then explicitly removed

A real gate (a new device had to be approved by an existing one before it could sign in, `PendingDeviceLogin` in `packages/database/prisma/schema.prisma`) was built at one point per a direct request, then just as directly asked to be removed again — logging in from a new device now always completes immediately in the same request, the same as the account's very first device always has via invite redemption. `PendingDeviceLogin`'s table/enum are left in the schema, unused (same "kept for historical/documentation reasons, not wired into any route" precedent `MessageReaction` already established) rather than dropped via a migration — nothing reads or writes it any more.
