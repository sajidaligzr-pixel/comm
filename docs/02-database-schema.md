...
| deletion_reason | enum(`manual`,`disappearing_timer`,`media_retention`) null | shipped alongside media retention (docs/10-privacy-data-retention.md) — set the moment `deleted_at` is, purely so the client can show "deleted" vs. "disappeared" vs. "expired" instead of one generic tombstone message for all three paths |
