...
| Calling | WS `call.invite`, `group-call.start` | Strict (6/min each) — ringing someone (or a whole group) is disruptive in a way a chat message isn't; well above any legitimate retry cadence but stops an account being used to harass via repeated rings |
...
| Group call signaling | WS `group-call.join`/`leave`/`offer`/`answer`/`ice-candidate` | Generous, deliberately more so than 1:1 (400/min) — a mesh call's signaling volume scales with participant count in a way 1:1's never does: one join near a full room opens up to `GROUP_CALL_MAX_PARTICIPANTS - 1` simultaneous offer/answer/ICE exchanges instead of 1:1's single pair |
...
## Search
<a id="search"></a>
There is no server-side message search endpoint — structurally impossible by design, since message content is ciphertext at rest. **Shipped** (docs/13-roadmap.md's WhatsApp-parity feature batch) as a 100% client-side linear scan over the already-loaded local decrypted cache, scoped to one open conversation (`messageMatchesSearch`, `apps/web/lib/message-content.ts` / the equivalent in `apps/mobile`'s `thread_screen.dart`) — deliberately not a real index; cross-conversation global search stayed out of scope for this pass. Either way, plaintext content never reaches the server — see master-prompt §30 and [09-trust-boundaries](09-trust-boundaries.md).
